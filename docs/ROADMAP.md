...
- `add_page_widget`;
- `rename_page`;
- bounded `update_page_widget` title/description/native chart-type/saved-sort/select-by/custom-widget-settings behavior, including explicit description clearing, chart-only enforcement and normalized post-write verification;
...
- the active bounded-grid-display PR adds normalized table-widget `gridOptions` and bounded mutation for vertical/horizontal gridlines, zebra stripes and row-number mode (`number`, `rowId`, `hidden`), preserving all untargeted widget options and verifying the complete expected options object after re-read. This item is not integrated until that PR is merged.

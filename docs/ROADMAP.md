...
Current integrated J2 implementation/evidence state at `main` `7ada2b6b56a9db2f5cecf2fb8462cf177d508e57`:
...
- The next useful J2 action is therefore operational execution of the integrated internal observer/provisioner against the disposable owner-controlled fixture, followed by durable evidence. It requires an environment that can run the current repository code with the fixture owner credential and server-held synthetic LinkKeys; do not reimplement another public ACL surface to bypass that boundary.

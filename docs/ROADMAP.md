...
Q0 established the post-audit runtime trust baseline. The frozen Agentic Builder vision is integrated, J0 and J1 have both passed exact-main integrated completion reviews, and J1's finite contractual-execution proof is exhausted. J2 is now the next product dependency. Its one-trace, no-reassignment business semantics are accepted and the Design Compression Review from PR #159 is integrated. J2 now prioritizes one real bounded J1-backed schema/UI transformation on a controlled realistic copy, with targeted access/browser proof over the affected closure, rather than making a synthetic ACL/browser subsystem a prerequisite to the transformation. Critical LinkKey ACL and teacher-specific date behavior remain unverified and must stay `UNKNOWN` until controlled evidence exists. J3-J6 remain dependency-gated exactly as defined below; old deferred P5/P6 ideas do not become eligible merely because the Builder program exists.
...
- J0 and J1 have passed their required integrated reviews and are DONE; J2's compressed reference-transformation path is ACTIVE, and isolated transformation engineering may proceed on a safely controlled copy while targeted browser evidence is prepared; reference-specific completion still requires current evidence for every impacted critical access/UI property;
...
J1 contractual execution      DONE
             |
             v
J2 stage-tracking reference application  ACTIVE — transform first, targeted evidence
             |
             v
...
**Status: ACTIVE — compressed end-to-end proof path.**

Goal: demonstrate one realistic cross-cutting application transformation covering schema, preservation of the existing access policy, UI, human-change preservation, concurrency and recovery. The proof is centered on the actual Builder transformation, not on building a general ACL or browser-testing subsystem. An ACL **write** is not required merely to exercise access-policy verification; add one only if the bound policy and accepted plan actually require it.

Authoritative behavioral specification: `docs/BEHAVIORAL-CONTRACT-STAGE-TRACKING.md`. Accepted business decisions: `docs/J2-STAGE-TRACKING-ACCEPTED-SEMANTICS.md`. Partially observed reference: `docs/J2-STAGE-TRACKING-REFERENCE-BINDING.md`. Design-compression rationale: `docs/J2-DESIGN-COMPRESSION-REVIEW.md`.

The assigned teacher writes one mutable call/visit trace on the existing `Stages` row and is its business author when recording a contact. The accepted workflow has no reassignment: contact entry, correction, clearing and transformation leave `Suivi_par` unchanged. A separate historical-author field is not required. No separate Visit/contact history or technical editor audit is required. `Date_du_contact` was added directly to the live reference and reported visible by the owner; this is reference state, not a J1/Builder execution proof.

The BehavioralContract remains the **independent test oracle**. Its application-wide properties are not weakened. J2 acceptance, however, requires current evidence for the `CRITICAL` properties reached by the exact transformation's dependency closure; broader accepted properties such as LinkKey revocation remain `UNKNOWN` rather than becoming mandatory transformation tests when the date/UI change does not reach them.

Current integrated J2 implementation/evidence state through `main` `daa7d13c9fa44c10ec90f659661ce7d762f24050`:

- bounded internal AccessModel observation/probe support, synthetic ACL/LinkKey provisioning support and the controlled-browser verifier core are integrated diagnostic/test infrastructure; none of them is a public MCP capability and none by itself proves the reference policy;
- the date-present/human-modified fixture observation is useful structural evidence, but the current fixture is model-facing readable and therefore **must not receive LinkKeys**;
- the fixed synthetic 13-rule policy/provisioner is not accepted evidence that the reference access policy was preserved; installing it is no longer a prerequisite to J2 transformation work;
- PR #156's browser verifier core may be reused for bounded evidence, but expanding it into a browser platform is not a J2 goal;
- PR #158 is Draft with an exact-head `CHANGES REQUIRED`: its custom CDP transport is **not eligible in its current scope as a J2 prerequisite**. After this scope change is integrated it may be reduced to the smallest reusable teacher-browser path, or closed as a superseded experiment;
- missing live-reference ACL/UI parity does not block isolated transformation engineering on a safe controlled copy, but it prevents reference-specific `VERIFIED` claims for dependencies that remain unknown.

Finite committed J2 work, in dependency order where a dependency is real:

1. **J2-R1 — relevant reference binding and safe controlled copy.** With owner authority, observe only the schema/UI/access/share/link dependencies reached by the contact-date change. Prepare one disposable sanitized realistic application with fictional business rows and the relevant existing policy/UI, or record reference-specific parity as `UNKNOWN`. Create two independent initial checkpoints: date absent, and date already present with legitimate human layout/data edits. Do not invent a replacement ACL policy. No real student/teacher rows or LinkKeys enter the repository/model surface.
2. **J2-T1 — actual J1-backed bounded transformation.** Implement the smallest engine effect adapters needed to add `Date_du_contact` when absent and expose it in the intended teacher-facing UI, with immutable contract/plan identity, fresh authority/preconditions, cumulative budget, write-ahead intent, exact target/effect knowledge, bounded recovery and postconditions. Run the same accepted intent first against the date-absent checkpoint and first against the independent date-present/human-edited checkpoint; adopt/preserve existing state instead of duplicating or resetting it. Leave `Suivi_par` and business rows unchanged. Do not mutate ACLs when the observed Stage policy already protects the new field; if an ACL write is actually necessary, specify/review that bounded effect separately before implementing it.
3. **J2-V1 — targeted browser regression over the affected closure.** On a secret-safe controlled copy, prove the responsible-teacher flow can reach/edit/correct/clear the date on the same Stage, prove a distinct unassigned teacher cannot read/edit the protected trace on the affected teacher page or Raw Data, and prove a no-key session exposes no protected follow-up. Add invalid-key, revocation, relation-tampering or additional-view cases only when the ImpactGraph reaches those properties. Browser evidence is contextual to the tested copy/revision/version and never substitutes owner/API reads for user-policy proof.
4. **J2-R2 — interruption, concurrency and true rerun.** Inject at least one response-loss/restart boundary and one relevant concurrent human UI/configuration edit, prove J1 recovery/suspension semantics and preservation of legitimate changes, then rerun the accepted intent on already managed state and prove convergence without duplicate field/widget/Stage/trace.
5. **J2 integrated completion review.** On exact `main`, independently challenge the intent-to-plan-to-effect-to-evidence chain, property/impact closure, safe secret isolation, reference/copy parity, targeted browser negatives, recovery/concurrency behavior and rerun. Mark J2 DONE only when every impacted critical property has current `VERIFIED` evidence and no committed J2 work remains. Unverified broader application properties must remain explicit; do not broaden the test program merely to erase unrelated `UNKNOWN`s.

J2-R1 and bounded J2-T1 implementation may progress in parallel where the exact effect semantics do not depend on missing reference policy facts. J2-V1 can be prepared alongside them and runs once the controlled secret-bearing copy is safe. A broad browser/ACL harness is not a dependency of J2-T1.

Secret isolation invariant: while synthetic LinkKeys exist, the test application must be unreachable through every model-facing bridge read path. Prefer a separate test origin with no bridge. A same-origin run is admissible only in a controlled exclusive window with stable access configuration, a strongest-path negative read before token creation, no return to bridge reach while tokens exist, verified token cleanup and a final negative read. A momentary 403, authentication failure or `UNKNOWN` probe is not isolation evidence. The currently bridge-readable fixture remains barred from LinkKeys.
...
```text
Worker A: J2-R1 relevant binding/safe copy or J2-T1 bounded transformation
...
J2-R1 and bounded J2-T1 implementation are eligible without first completing a broad synthetic ACL/browser harness. J2-V1 is the targeted affected-path browser proof and may be prepared alongside them; it runs only on a secret-safe controlled copy. Reference-specific completion still requires current parity/evidence for every impacted critical dependency. J1 isolated engineering is complete, and no second real user or production Builder deployment may rely on the shared static Grist credential path as if it provided per-user isolation.
...
2. progress J2-R1/J2-T1 toward the actual bounded transformation; prepare/run J2-V1 only at the smallest affected browser boundary and do not revive #158's broad custom-browser scope as a prerequisite;

...
`ExecutionLifecycle.prepareEffect()` is the write-ahead barrier for an effectful step:
...
5. only after `prepareEffect()` resolves may a caller dispatch the external effect.

`ExecutionLifecycle.recordEffect()` then durably records effect knowledge. `UNCERTAIN` and `PARTIALLY_APPLIED` outcomes suspend the execution immediately; this slice contains no capability-specific automatic replay rule.
...
The lifecycle deliberately does not accept an effect callback. This keeps the write-ahead boundary explicit: external dispatch is forbidden until the returned journal record proves that the `RUNNING` state has been persisted.

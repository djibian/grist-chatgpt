...
3. **Verify effective behavior (J2-C).** Use separate non-owner browser sessions for assigned teacher A, other teacher B, missing/invalid and revoked links. Check reading and writing through the supported teacher page, other reachable pages and Raw Data. A records, corrects and clears one contact without changing `Suivi_par` or the Stage identity; B's separate link stays denied. Check denied read/write, relation tampering, date display and field mapping. A synthetic pass does not by itself verify the live DINUM teacher link.
4. **Transform through J1 (J2-D).** Record the exact `MANAGED`/`SHARED` regions, formulas, access dependencies and integrations in the accepted plan. On isolated fixtures only, add/expose the date where absent; leave `Suivi_par` unchanged and do not add a historical-author field for this workflow. Reconcile the already present date and human layout in the other state, and verify existing rows, interruption, concurrency and rerun. Add an ACL write only if observed policy requires it, with bounded authorization and recovery. The direct live schema edit is not Builder proof.
...
|---|---|---|
| `STAGE-B1/B2/B10`, `STAGE-U1` | A enters, corrects and clears one trace while the Stage and assignment A stay unchanged; B's distinct link stays denied and the contact date remains reachable where authorized. | Non-owner controlled browser and stable Stage identity/assignment before and after each edit. |

...
`update_page_widget` may additionally mutate the bounded `customWidgetSettings` subset for an explicitly identified existing `type === "custom"` widget. Public inputs are limited to access (`none`, `read table`, `full`) and column mappings expressed only with exact stable current column IDs (single/list/null). Mapping input is bounded to 100 keys, 1,000 mapped columns and a 5,000-column schema ceiling. The bridge resolves internal numeric refs server-side, read-modify-writes the complete existing `options` object while changing only `customView.access` / `customView.columnsMapping`, and re-reads the widget to verify the complete expected options object. URL, plugin/widget identity and arbitrary widget-owned options are deliberately not writable model inputs. A post-write mismatch is a `write_verification_failed` condition and must not be blindly retried.
